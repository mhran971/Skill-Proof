            ...
            <Card key={u.rank} className={`overflow-hidden ${i===0?"md:-translate-y-2":""}`}>
              ...
              <CardContent className="flex flex-col items-center p-6 text-center">
                <div className="relative">
                  <Avatar className="h-20 w-20 border-4 border-card shadow-elegant"><AvatarImage src={u.avatar}/><AvatarFallback>{u.name[lang].charAt(0)}</AvatarFallback></Avatar>
                ...
                <div className="mt-4 font-bold">{u.name[lang]}</div>
                <Badge className="mt-2 bg-gradient-primary text-primary-foreground">{u.badge}</Badge>
                <div className="mt-3 text-2xl font-extrabold text-gradient">{u.score.toLocaleString()}</div>
      ...
      <Card className="mt-6">
        <CardContent className="p-0">
          <div className="divide-y">
            {rest.map(u=>(
              <div key={u.rank} className="flex items-center gap-4 p-4">
                <div className="grid h-8 w-8 shrink-0 place-items-center rounded-full bg-muted text-sm font-bold">{u.rank}</div>
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <Badge variant="outline" className="mt-0.5 text-[10px]">{u.badge}</Badge>
                </div>
                <div className="text-end">
                  <div className="font-extrabold">{u.score.toLocaleString()}</div>
